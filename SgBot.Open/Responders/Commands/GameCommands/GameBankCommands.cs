...
namespace SgBot.Open.Responders.Commands.GameCommands
{
    public static partial class BotGameCommands
    {
            ...
            GroupMessageReceiver groupMessageReceiver)
        {
            ...
            if (player.Coin < need)
            {
                await groupMessageReceiver.QuoteMessageAsync($"金币不足{player.PowerNeedCoin(what) * player.Level}\n" +
            ...
            await groupMessageReceiver.QuoteMessageAsync($"{player.Name}({player.Id})购买{what} 体力成功");
        }
        /// <summary>
        /// 用傻狗力购买金币
        /// </summary>
            ...
            GroupMessageReceiver groupMessageReceiver)
        {
            ...
            await DataBaseOperator.UpdatePlayer(player);
            await groupMessageReceiver.QuoteMessageAsync($"{player.Name}({player.Id})购买 {what * player.Level} 金币成功");
        }
        /// <summary>
        /// 用金币提取傻狗力
        /// </summary>
            ...
            GroupMessageReceiver groupMessageReceiver)
        {
            ...
            {
                await groupMessageReceiver.QuoteMessageAsync($"金币不足{what * player.Level * 2} 1傻狗力需要 2倍玩家等级 的金币");
                return;
            }
            ...
            await DataBaseOperator.UpdateUserInfo(GroupMessageReceivedInfo.Member);
            await DataBaseOperator.UpdatePlayer(player);
            await groupMessageReceiver.QuoteMessageAsync($"{player.Name}({player.Id})提取 {what} 傻狗力成功");
        }
    }
}
